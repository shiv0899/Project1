...
public class GlobalServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (ss1.equals("adminlogin")) {
			...
			WhatsappDAOInterface id = ControllerFactory.createObjectHibernate();
			boolean b = id.loginadminProfile(ic);
			// System.out.println(b);
			...
			if (b) {
				...
				RequestDispatcher rd = getServletContext().getRequestDispatcher("/GlobalServlet?choice=successadmin");
				rd.forward(request, response);
		...
		if (ss1.equals("deleteadmin")) {
			...
			PrintWriter out = response.getWriter();
			out.println("<html><body><center><br><br>");
			RequestDispatcher rd = getServletContext().getRequestDispatcher("/deleteadmin.jsp");
			rd.include(request, response);
			...
			int i = ww.admindeleteProfile(ws);
			out.println("<html><body><center><br><br>");
			
			if (i > 0) {

				out.println("<font=5 style='color:orange'><b>Profile Deleted Succesfully. We miss you.......</b></font>");
				try {
				Thread.sleep(5000);
				}
				catch(Exception e)
				...
				RequestDispatcher rd1 = getServletContext().getRequestDispatcher("/admin.jsp");
				//response.sendRedirect("searchprofile.jsp");  
				rd1.forward(request, response);	
		...
		if (ss1.equals("view")) {
			...
			if (ww != null) {
				...
				out.println("<tr><td style=' border:1px solid black' width=100> User Name      :-</td><td style='border:1px solid black'> " + ww.getName() + "</td><tr>");
		...
		if(ss1.equals("search"))
		{
			...
			if(w!=null) {
				...
				out.println("<tr><th>Sr.No</th><th  width:100px>Name</th><th>Email</th><th>Age</th><th>Address</th><th>Password</th></tr>");	
			out.println("<tr><td style='border:1px solid white'>Name Of User:-</td><td style='border:1px solid white'>"+w.getName()+"</td></tr>");
			out.println("<tr><td style='border:1px solid white'>Age Of User:-</td><td style='border:1px solid white'>"+w.getAge()+"</td></tr>");
			out.println("<tr><td style='border:1px solid white'>Address Of User:-</td><td style='border:1px solid white'>"+w.getAddress()+"</td></tr>");
		...
		if(ss1.equals("viewall"))
		{WhatsappDAOInterface id = ControllerFactory.createObjectHibernate();
		
		response.setContentType("text/html");
		PrintWriter out = response.getWriter();
		out.println("<html><body><br><br><center>");
			List<WhatsappUser> ww2 = id.allUser();
			RequestDispatcher rd = getServletContext().getRequestDispatcher("/viewall.jsp");
			//response.sendRedirect("searchprofile.jsp");  
			rd.include(request, response);
			//out.println("<font size=10 color= ><b> -:Users Profile:-</b></font><br><br>");
			for(WhatsappUser ww3:ww2) {
					
				
				out.println("<br><br><font size=5 color=orange ><b> -:Users Profile:-</b></font><br><br>");
				out.println("<center><table border=3 width=100 >");
				
				out.println("<tr><td style=' border:1px solid black' width=400> User Name      :-</td><td style='border:1px solid black'> " + ww3.getName() + "</td><tr>");
				out.println("<tr><td style='border:1px solid black' > User Email     :-</td><td style='border:1px solid black'>" + ww3.getEmail() + "</td><tr>");
				out.println(" <tr><td style='border:1px solid black'>User Age       :- </td><td style='border:1px solid black'>" + ww3.getAge() + "</td><tr>");
				out.println("<tr><td style='border:1px solid black'> User Address   :- </td><td style='border:1px solid black'>" + ww3.getAddress() + "</td><tr>");
				out.println(" <tr><td style='border:1px solid black'>User Password  :- </td><td style='border:1px solid black's>" + ww3.getPassword() + "</td><tr>");
